...
public class ArrayListAlgorithms
{
    ...
    public static void duplicateUpperEnd(ArrayList<String> wordList) {
        ...
        for (int i = 0; i < origLength; i++) {
            wordList.add(i + origLength, wordList.get(i).toUpperCase());
        }
    }
    ...
    public static ArrayList<String> parseWordsAndReverse(String sentence) {
        ...
        int tempValue = wordList.size() - 1;
        String tempString;
        for (int i = 0; i < wordList.size() / 2; i++) {
            tempString = wordList.get(i);
            wordList.set(i, wordList.get(tempValue));
            wordList.set(tempValue, tempString);
            tempValue--;
        }
        return wordList;
    }
    ...
    public static void removeDuplicates(ArrayList<Integer> intList) {
        for (int i = 0; i < intList.size(); i++) {
            for (int z = i + 1; z < intList.size(); z++) {
                if (intList.get(i) == intList.get(z)) {
                    intList.remove(z);
                    z--;
                }
            }
    ...
    public static void moveBWords(ArrayList<String> wordList) {
        ...
        for (int i = 0; i < wordList.size(); i++) {
            if (wordList.get(i).contains("b")) {
                wordList.add(counter, wordList.get(i));
                wordList.remove(i + 1);
                counter++;
            }
        }
